...
void runDbscanThread(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, float eps,
                     int minPts) {
  DBSCAN dbscan(cloud, eps, minPts);
  dbscan.runDbscan();
}

void createClusterBoundaryMarkers(pcl::PointCloud<pcl::PointXYZI>::Ptr cluster,
                                  int clusterId,
...
void filterPointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr &inputCloud,
...
void cloudCallback(const sensor_msgs::PointCloud2ConstPtr &cloudMsg) {
  pcl::PointCloud<pcl::PointXYZ>::Ptr cCloud(
  ...
  vg.setLeafSize(0.5f, 0.5f,
                 0.5f); // Adjust for down-sampling // default: 0.2f, 0.2f, 0.2f
  pcl::PointCloud<pcl::PointXYZ>::Ptr filteredCloud(
      new pcl::PointCloud<pcl::PointXYZ>());
  ...
  ROS_INFO("Filtered cloud size after PassThrough: %lu",
           rangeFilteredCloud->points.size());
  if (rangeFilteredCloud->empty()) {
    ROS_WARN(
        "Filtered cloud is empty after PassThrough. Adjust the filter limits.");
    return;
  }
  ...
  for (const auto &cluster : clusters) {
    pcl::PointCloud<pcl::PointXYZI>::Ptr clusterCloud(
        new pcl::PointCloud<pcl::PointXYZI>());
    clusterCloud->points.insert(clusterCloud->points.end(),
                                cluster.second.begin(), cluster.second.end());
    createClusterBoundaryMarkers(clusterCloud, clusterId++, cloudMsg->header,
                                 markerArray);
  }
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subCloud = nh.subscribe(
      "/processed_cloud", 1,
      cloudCallback); // default: "/point_cloud" or "/trans_depth_cloud"
  pubClusteredCloud =
      nh.advertise<sensor_msgs::PointCloud2>("/dbscan_clusters", 1);
  pubMarkers = nh.advertise<visualization_msgs::MarkerArray>(
      "/dbscan_cluster_centroids", 1);
